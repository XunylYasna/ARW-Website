...
const Logo = (link) => <img className="logo" alt="logo" src={link.value} />;
const ViewPage = (slug) => (
  <AniLink className="button" to={"/organizations/" + slug.value}>
    <div id="dub-arrow">
      <img
    ...
    <Layout pageName="organizations">
      ...
      <Griddle
        ...
        components={{
          ...
          PageDropdown: (page) =>
            page.maxPages !== 0 ? (
              <div className="page-identifier">
                Page {page.currentPage}/{page.maxPages}
              </div>
            ) : (
              <span />
            ),
          NextButton: (next) =>
            next.hasNext ? (
              <div className={next.className} onClick={next.onClick}>
                <GrFormNext />
              </div>
            ) : (
              <span />
          ...
          PreviousButton: (prev) =>
            prev.hasPrevious ? (
              <div className={prev.className} onClick={prev.onClick}>
                <GrFormPrevious />
              </div>
            ) : (
              <span />

...
export default function ClusterPage({ data }) {
  ...
  useEffect(() => {
    ...
    organizationsTimeline
      .staggerFromTo(
        ".organization-list .sub-line",
        0.5,
        { scaleX: 0 },
        { scaleX: 1, transformOrigin: "left" }
      )
      .staggerFromTo(
        ".organization-list .main-header",
        0.5,
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0 }
      )
  ...
  const {
    ...
    buildingSize,
    organizations,
  } = data.allContentfulCluster.nodes[0];
  return (
    <Layout pageName="cluster" mainName={title}>
      ...
      <Minimap
        minimap={landingImage.fluid.src}
                    ...
                    >
                      <div className="org-item">
                        <img
                          className="org-logo"
                          draggable={false}
                          src={org.logo.fluid.src}
                          alt={org.acronym + " Logo"}
                        />
                        <h1 className="sub-title">
                          {org.organizationName}(<strong>{org.acronym}</strong>)
                        </h1>
                      </div>
                    </AniLink>
                  </Card>
